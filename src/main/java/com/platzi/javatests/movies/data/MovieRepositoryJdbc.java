...
public class MovieRepositoryJdbc implements MovieRepository {
    ...
    public Collection<Movie> findByName(String name) {
        Object[] args = { name };
        return jdbcTemplate.query("select * from movies where LOWER(name) LIKE CONCAT('%',LOWER(?),'%') > 0", args, movieMapper);
    }

    public Collection<Movie> findByDirector(String director) {
        Object[] args = { director };
        return jdbcTemplate.query("select * from movies where LOWER(director) LIKE CONCAT('%',LOWER(?),'%') > 0", args, movieMapper);
    }
}
